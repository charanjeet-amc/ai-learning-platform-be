...
public class SeedController {
    ...
    @PostMapping("/seed-extra-questions")
    @Transactional
    public ResponseEntity<Map<String, Object>> seedExtraQuestions() {
        // Check if non-MCQ questions already exist
        long nonMcqCount = questionRepository.findAll().stream()
                .filter(q -> q.getType() != QuestionType.MCQ)
                .count();
        if (nonMcqCount > 0) {
            return ResponseEntity.ok(Map.of("message", "Extra questions already seeded", "nonMcqCount", nonMcqCount));
        }
        ...
        return ResponseEntity.ok(Map.of("message", "Extra questions seeded successfully", "count", count));
    }
